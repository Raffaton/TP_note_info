...
int resoudre_grille(int grille[9][9], int taille, int ligne, int colonne) {
    if (ligne == taille) {
        return 1;
    }

    if (colonne == taille) {
        return resoudre_grille(grille, taille, ligne + 1, 0);
    }

    if (grille[ligne][colonne] != 0) {
        return resoudre_grille(grille, taille, ligne, colonne + 1);
    }

    for (int nombre = 1; nombre <= taille; nombre++) {
        if (verification_grille(ligne, colonne, grille, taille, nombre, 0)) {
            grille[ligne][colonne] = nombre;
            ...
            if (resoudre_grille(grille, taille, ligne, colonne + 1)) {
                return 1;
            }
            else
            {
...
int main() {
    ...
    printf("Voulez-vous resoudre ce tableau automatiquement (0 pour NON / 1 pour OUI) ? ");
    scanf("%d", &choix_resolution);

    if (choix_resolution == 0) {
        if (grille_est_vide(grille, taille)) {
            printf("Erreur : La grille est vide, il est donc impossible de la resoudre !\n");
        } else {
            if (resoudre_grille(grille, taille, 0, 0)) {
                printf("Grille resolue avec succes :\n");
                afficher_grille(taille, grille);
            } else {
                printf("Aucune solution n'a pu etre trouvee.\n");
            }
        }
    }
    else 
    {
        ...
        if (resoudre_grille(grille, taille, 0, 0)) 
        {
            printf("Felicitations ! La grille a ete resolue automatiquement !\n");
        } 
        else 
        {
            printf("Impossible de resoudre cette grille.\n");
        }
    }
